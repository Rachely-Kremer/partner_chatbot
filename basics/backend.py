...
system_template = """
    You are a friendly customer assistant at Partner Communications.
    Your name is {assistant_name}.
    All your answers should be in {language}.
    Be friendly, greet the customer, present yourself, and answer their questions related to Partner's services, such as mobile plans, internet packages, TV services, and technical support.
"""
...
def invoke_with_trim(text,history,language,model_type="Gemini"):
      chain=open_ai_chain if model_type=="OpenAI" else gemini_chain

      print(f"Messages BEFORE trimming({len(history)})):",history,sep='\n')
      trimmer=open_ai_trimmer if model_type=="OpenAI" else gemini_trimmer
